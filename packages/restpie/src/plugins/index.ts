  ...
  for (const p of allPaths) {
    ...
    for (const filename of fs.readdirSync(p)) {
      try {
        ...
        // Not an RestPie plugin because it doesn't have the package.json['insomnia']
...
export async function getPlugins(force = false): Promise<Plugin[]> {
  ...
  if (!plugins) {
    ...
    const pluginPath = path.join(process.env['RESTPIE_DATA_PATH'] || (process.type === 'renderer' ? window : electron).app.getPath('userData'), 'plugins');
