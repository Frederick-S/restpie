...
export function createWindow() {
  ...
  const helpMenu: MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: () => {
          const directory = process.env['RESTPIE_DATA_PATH'] || electron.app.getPath('userData');
  ...
  if (isDevelopment() || process.env.RESTPIE_FORCE_DEBUG) {
...
async function showUnresponsiveModal() {
  const id = await dialog.showMessageBox({
    ...
    cancelId: 0,
    title: 'Unresponsive',
    message: 'RestPie has become unresponsive. Do you want to reload?',
  });
...
function initLocalStorage() {
  const localStoragePath = path.join(process.env['RESTPIE_DATA_PATH'] || electron.app.getPath('userData'), 'localStorage');
