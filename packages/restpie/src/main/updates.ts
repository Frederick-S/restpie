...
const isUpdateSupported = () => {
  ...
  if (process.env.RESTPIE_DISABLE_AUTOMATIC_UPDATES) {
    console.log('[updater] Disabled by RESTPIE_DISABLE_AUTOMATIC_UPDATES environment variable');
    return false;
  }
  if (isDevelopment()) {
...
export const init = async () => {
  ...
  autoUpdater.on('update-downloaded', async (_error, releaseNotes, releaseName) => {
    ...
    dialog.showMessageBox({
      ...
      detail: 'A new version of RestPie has been downloaded. Restart the application to apply the updates.',
    }).then(returnValue => {
