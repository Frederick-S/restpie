...
interface InsomniaPlugin {
  // RestPie attribute from package.json
  insomnia: {
    name: string;
    displayName: string;
    description: string;

    // Used by the plugin hub, not currently used by RestPie
...
export default async function(lookupName: string) {
  return new Promise<void>(async (resolve, reject) => {
    ...
    try {
      ...
      const pluginDir = path.join(process.env['RESTPIE_DATA_PATH'] || electron.app.getPath('userData'), 'plugins', moduleName);
  ...
  return new Promise<InsomniaPlugin>((resolve, reject) => {
    ...
    childProcess.execFile(
      ...
      (err, stdout, stderr) => {
        ...
        console.log(`[plugins] Detected RestPie plugin ${data.name}`);
