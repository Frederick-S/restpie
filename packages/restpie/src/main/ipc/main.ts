...
import { restpieFetch } from '../restpieFetch';
...
export interface MainBridgeAPI {
  ...
  restpieFetch: typeof restpieFetch;
  showContextMenu: (options: { key: string }) => void;
}
export function registerMainHandlers() {
  ipcMain.handle('restpieFetch', async (_, options: Parameters<typeof restpieFetch>[0]) => {
    return restpieFetch(options);
  });
