...
const getDataDirectory = () => process.env.RESTPIE_DATA_PATH || electron.app.getPath('userData');
...
export const createConfiguredCurlInstance = ({
  ...
  const userAgentOrFallback = typeof userAgent?.value === 'string' ? userAgent?.value : 'restpie/' + version;
