...
export const GIT_RESTPIE_DIR_NAME = '.restpie';
export const GIT_INTERNAL_DIR = path.join(GIT_CLONE_DIR, gitInternalDirName);
export const GIT_RESTPIE_DIR = path.join(GIT_CLONE_DIR, GIT_RESTPIE_DIR_NAME);
...
export class GitVCS {
  ...
  async listFiles() {
    ...
    const insomniaFiles = repositoryFiles
      .filter(file => file.startsWith(GIT_RESTPIE_DIR_NAME))
      .map(convertToOsSep);
