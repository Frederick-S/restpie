...
import { GIT_CLONE_DIR, GIT_RESTPIE_DIR, GIT_RESTPIE_DIR_NAME } from '../git-vcs';
...
describe('NeDBClient', () => {
  ...
  describe('readdir()', () => {
    it('reads model IDs from model type folders', async () => {
      ...
      const reqDir = path.join(GIT_RESTPIE_DIR, models.request.type);
      const wrkDir = path.join(GIT_RESTPIE_DIR, models.workspace.type);
      expect(await neDbClient.readdir(GIT_CLONE_DIR)).toEqual([GIT_RESTPIE_DIR_NAME]);
      expect(await neDbClient.readdir(GIT_RESTPIE_DIR)).toEqual([
        models.apiSpec.type,
  ...
  describe('readFile()', () => {
    it('reads file from model/id folders', async () => {
      const wrk1Yml = path.join(GIT_RESTPIE_DIR, models.workspace.type, 'wrk_1.yml');
      const req1Yml = path.join(GIT_RESTPIE_DIR, models.request.type, 'req_1.yml');
      const reqXYml = path.join(GIT_RESTPIE_DIR, models.request.type, 'req_x.yml');
  ...
  describe('stat()', () => {
    it('stats a dir', async () => {
      ...
      expect(await neDbClient.stat(GIT_RESTPIE_DIR)).toEqual(dirType);
  ...
  describe('writeFile()', () => {
    it('should ignore files not in GIT_RESTPIE_DIR directory', async () => {
      // Arrange
    ...
    it('should write files in GIT_RESTPIE_DIR directory to db', async () => {
      // Arrange
      const workspaceId = 'wrk_1';
      const neDbClient = new NeDBClient(workspaceId, DEFAULT_PROJECT_ID);
      const upsertSpy = jest.spyOn(db, 'upsert');
      ...
      const filePath = path.join(GIT_RESTPIE_DIR, env.type, `${env._id}.yml`);
      // Act
      await neDbClient.writeFile(filePath, YAML.stringify(env));
      // Assert
    ...
    it('should set workspace parentId to the project', async () => {
      ...
      const filePath = path.join(GIT_RESTPIE_DIR, models.workspace.type, `${workspaceId}.yml`);
    ...
    it('should throw error if id does not match', async () => {
      ...
      const filePath = path.join(GIT_RESTPIE_DIR, env.type, 'env_2.yml');
      // Act
    ...
    it('should throw error if type does not match', async () => {
      ...
      const filePath = path.join(GIT_RESTPIE_DIR, models.request.type, `${env._id}.yml`);
      // Act
      const promiseResult = neDbClient.writeFile(filePath, YAML.stringify(env));
      // Assert
      await expect(promiseResult).rejects.toThrowError(
