...
import GitVCS, { GIT_CLONE_DIR, GIT_RESTPIE_DIR } from '../git-vcs';
...
describe('Git-VCS', () => {
  ...
  beforeAll(() => {
    fooTxt = path.join(GIT_RESTPIE_DIR, 'foo.txt');
    barTxt = path.join(GIT_RESTPIE_DIR, 'bar.txt');
  });
  ...
  describe('common operations', () => {
    ...
    it('stage and unstage file', async () => {
      const fsClient = MemClient.createClient();
      await fsClient.promises.mkdir(GIT_RESTPIE_DIR);
      ...
      expect(await GitVCS.log()).toEqual([
        {
          commit: {
            ...
            parent: [],
            tree: '76cd55ffdaaf605351b3a76aa8a0cb5b1fe6b66c',
          },
          oid: '35c94fc84d09085bbdb4e299630af24cdb47f054',
          payload: `tree 76cd55ffdaaf605351b3a76aa8a0cb5b1fe6b66c
...
`,
  ...
  describe('undoPendingChanges()', () => {
    it('should remove pending changes from all tracked files', async () => {
      const folder = path.join(GIT_RESTPIE_DIR, 'folder');
    ...
    it('should remove pending changes from select tracked files', async () => {
      const foo1Txt = path.join(GIT_RESTPIE_DIR, 'foo1.txt');
      const foo2Txt = path.join(GIT_RESTPIE_DIR, 'foo2.txt');
      const foo3Txt = path.join(GIT_RESTPIE_DIR, 'foo3.txt');
  ...
  describe('readObjectFromTree()', () => {
    it('reads an object from tree', async () => {
      const fsClient = MemClient.createClient();
      const dir = path.join(GIT_RESTPIE_DIR, 'dir');
