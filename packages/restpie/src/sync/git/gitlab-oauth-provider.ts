...
const getGitLabConfig = async () => {
  const { RESTPIE_GITLAB_REDIRECT_URI, RESTPIE_GITLAB_CLIENT_ID } = env;

  // Validate and use the environment variables if provided
  if (
    (RESTPIE_GITLAB_REDIRECT_URI && !RESTPIE_GITLAB_CLIENT_ID) ||
    (!RESTPIE_GITLAB_REDIRECT_URI && RESTPIE_GITLAB_CLIENT_ID)
  ) {
    throw new Error('GitLab Client ID and Redirect URI must both be set.');
  }

  if (RESTPIE_GITLAB_REDIRECT_URI && RESTPIE_GITLAB_CLIENT_ID) {
    return {
      clientId: RESTPIE_GITLAB_CLIENT_ID,
      redirectUri: RESTPIE_GITLAB_REDIRECT_URI,
    };
  }
...
export const getGitLabOauthApiURL = () =>
  env.RESTPIE_GITLAB_API_URL || 'https://gitlab.com';
