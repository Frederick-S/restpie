...
export function useVCS({
  ...
  useEffect(() => {
    ...
    async function updateVCS() {
      ...
      if (!vcsInstance) {
        const driver = FileSystemDriver.create(process.env['RESTPIE_DATA_PATH'] || window.app.getPath('userData'));
...
export const WorkspaceSyncDropdown: FC = () => {
  ...
  return <GitSyncDropdown isInsomniaSyncEnabled={false} gitRepository={gitRepository} />;
  // }
