...
const RESTPIE_NOTIFICATIONS_SEEN = 'restpie::notifications::seen';
...
export const Toast: FC = () => {
  ...
  const handleNotification = (notification: ToastNotification | null | undefined) => {
    ...
    try {
      const storedKeys = window.localStorage.getItem(RESTPIE_NOTIFICATIONS_SEEN);
      if (storedKeys) {
    ...
    window.localStorage.setItem(RESTPIE_NOTIFICATIONS_SEEN, JSON.stringify(seenNotifications, null, 2));
  ...
  const checkForNotifications = async () => {
    ...
    try {
      ...
      // const notificationOrEmpty = await window.main.restpieFetch<ToastNotification>({
      //   method: 'POST',
      //   path: '/notification',
      //   data,
