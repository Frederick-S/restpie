...
export const gitRepoAction: ActionFunction = async ({
  ...
  try {
    ...
    const baseDir = path.join(
      process.env['RESTPIE_DATA_PATH'] || window.app.getPath('userData'),
      `version-control/git/${gitRepository._id}`
    );

    // All app data is stored within a namespaced GIT_RESTPIE_DIR directory at the root of the repository and is read/written from the local NeDB database
...
export const cloneGitRepoAction: ActionFunction = async ({
  ...
  const containsInsomniaDir = async (
    ...
    return rootDirs.includes(GIT_RESTPIE_DIR_NAME);
  };
  ...
  if (!(await containsInsomniaWorkspaceDir(fsClient))) {
    ...
    const workspace = await models.workspace.create({
      ...
      description: `RestPie Workspace for ${repoSettingsPatch.uri}}`,
    });
    ...
    const workspaceBase = path.join(GIT_RESTPIE_DIR, models.workspace.type);
    ...
    for (const modelType of await fsClient.promises.readdir(GIT_RESTPIE_DIR)) {
      const modelDir = path.join(GIT_RESTPIE_DIR, modelType);
...
async function getGitVCSPaths(vcs: typeof GitVCS) {
  ...
  for (const type of await fs.readdir(GIT_RESTPIE_DIR)) {
    const typeDir = path.join(GIT_RESTPIE_DIR, type);
    for (const name of await fs.readdir(typeDir)) {
      // NOTE: git paths don't start with '/' so we're omitting
      //  it here too.
      const gitPath = path.join(GIT_RESTPIE_DIR_NAME, type, name);
...
async function getGitChanges(vcs: typeof GitVCS, workspace: Workspace) {
  ...
  for (const doc of docs) {
    ...
    statusNames[path.join(GIT_RESTPIE_DIR_NAME, doc.type, `${doc._id}.json`)] =
      name;
    statusNames[path.join(GIT_RESTPIE_DIR_NAME, doc.type, `${doc._id}.yml`)] =
      name;
  }
  // Create status items
