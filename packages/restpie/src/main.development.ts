...
const envDataPath = process.env.RESTPIE_DATA_PATH;
if (envDataPath) {
  ...
  const newPath = path.join(defaultPath, '../', isDevelopment() ? 'restpie-dev' : userDataFolder);
...
const defaultProtocol = `restpie${isDevelopment() ? 'dev' : ''}`;
...
const _launchApp = async () => {
  ...
  // Don't send origin header from RestPie because we're not technically using CORS
