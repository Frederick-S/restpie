...
export const randomDataPath = () => path.join(os.tmpdir(), 'restpie-smoke-test', `${uuidv4()}`);
export const RESTPIE_DATA_PATH = randomDataPath();

// Packaged app paths
const pathLookup: Record<string, string> = {
  win32: path.join('win-unpacked', 'RestPie.exe'),
  darwin: path.join('mac-universal', 'RestPie.app', 'Contents', 'MacOS', 'RestPie'),
  linux: path.join('linux-unpacked', 'restpie'),
};
export const cwd = path.resolve(__dirname, '..', '..', 'restpie');
const repoRoot = path.resolve(__dirname, '..', '..', '..');
const restpieBinary = path.join(cwd, 'dist', pathLookup[process.platform]);
const electronBinary = path.join(repoRoot, 'node_modules', '.bin', process.platform === 'win32' ? 'electron.cmd' : 'electron');

export const executablePath = bundleType() === 'package' ? restpieBinary : electronBinary;
...
const hasBinaryBeenBuilt = fs.existsSync(path.resolve(cwd, restpieBinary));
...
if (bundleType() === 'package' && !hasBinaryBeenBuilt) {
  console.error(`ERROR: ${restpieBinary} not found at ${path.resolve(cwd, restpieBinary)}
