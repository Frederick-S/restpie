...
This project contains the smoke testing suite for RestPie App.

> To find more about Inso CLI smoke tests, check [this document](CLI.md).

- [RestPie Smoke Tests](#restpie-smoke-tests)
...
```shell
# Example:
npx playwright show-trace packages/restpie-smoke-test/traces/app-can-send-requests/trace.zip
```
...
- RestPie console logs: `DEBUG=pw:browser npm run test:smoke:dev`
...
```shell
# Build executable in /packages/restpie/dist
npm run app-package
...
```shell
npm run test:dev -w packages/restpie-smoke-test -- preferences-interactions
```
